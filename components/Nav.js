...
const Nav = () => {
  ...
  const icon =
    theme === "light" ? (
      <BsMoonFill className="w-10 h-10" />
    ) : (
      <BsFillSunFill className="w-10 h-10" />
    );
    ...
    <div>
      <header className="border-b sticky top-0 z-20 bg-secondary dark:border-0 dark:bg-dark-100 dark:text-primary ">
        <div className="flex  items-center justify-between max-w-6xl pt-4 pb-2 px-4 mx-auto lg:max-w-screen-xl">
          ...
          <div className="dark:text-primary align-top pb-2 text-lg ">
            <a
              className="tet-md font-bold cursor-pointer  flex"
              onClick={() => setCartOpen(!cartOpen)}
            >
              <BsCart className="w-8 h-8 items-center justify-center mr-2 red-400 " />
              Cart ({cartQuantity})
            </a>
            <MiniCart cart={cart} />
          </div>
